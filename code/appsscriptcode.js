...
function fastSortLogs() {
  ...
  // If the sheet only has a header (or is empty), stop here.
  if (lastRow <= 1) {
    SpreadsheetApp.getUi().alert("No data found to sort below the header!");
    return;
  }
  
  // Define Range: Start at Row 2, Col 1, go to Last Row, Last Col
  const range = sheet.getRange(2, 1, lastRow - 1, lastCol);
  
  try {
    range.sort([
      {column: 1, ascending: true}, // Date
      {column: 3, ascending: true}, // Game
      {column: 4, ascending: true}  // Source
    ]);
    ss.toast("Data sorted: Date > Game > Source", "System Success", 2);
  } catch (e) {
